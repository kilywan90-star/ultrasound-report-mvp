    ...
    with open(TEMPLATE_CSV, encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            if len(info1) < 20:
                ...
                organ_kw = ['经','探查','位','大小','回声','见','mm','cm','正常','异常','切除','术后','全切']
...
def search_candidates(text: str, exam_type: str = "", limit: int = 10, category: str = None) -> list[dict]:
    ...
    for name in list(_template_index.keys()):
        info1 = _template_index[name].get("info1","")
        # 甲状腺全切术后已转换converted, 确保关键词能找到
        if "甲状腺全切术后" in name or "甲状腺全切" in name:
            scored[name] = max(scored.get(name, 0), 250)
    
    if "甲状腺全切术后" in text or "甲状腺全切" in text or "甲状腺已切除" in text:
        if "甲状腺全切术后" in _template_index:
            scored["甲状腺全切术后"] = 250
        # 如果没在索引中, 直接添加到keyword_index
        if "甲状腺全切术后" not in _keyword_index and "甲状腺全切术后" in _template_index:
            _keyword_index["甲状腺全切术后"] = "甲状腺全切术后"
    ...
    match_kw_dict = {
        ...
        "胆囊息肉":["胆囊","息肉","附壁","不移动"],
        "胆囊多发息肉样病变":["胆囊","息肉","多发息肉","附壁","不移动"],
        "甲状腺全切术后":["甲状腺全切术后","甲状腺全切","甲状腺术后","全切术后","甲状腺已切除"],
